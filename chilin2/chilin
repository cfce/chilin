...
"""
chilin2.modules --- a ChIP-seq pipeline for benchmarking quality control of the experiment
"""
...
def parse_args(args=None):
    ...
    parser_gen.add_argument("-o", "--output", dest = "out", help="name of config file, default: chilin.conf", default="chilin.conf")
    ...
    parser_simple = sub_parsers.add_parser("simple", help="run simple modes, should prepare species conf files when installing")
    ...
    parser_simple.add_argument("-s", "--species", dest = "species", help="species--from chilin.conf: %s" % species_ls, required=True)
    parser_simple.add_argument("-u","--user", dest="user", default="anonymous",
                               help="your name, DEFAULT: anonymous")

    parser_simple.add_argument("-r", "--rtype", dest = "rtype", help="input factor type for conservation", default="tf", choices=["tf", "histone", "dnase"])
    parser_simple.add_argument("-p", "--type", dest = "ptype", help="input peaks type for macs2 peaks calling, narrow, broad or both", default="both", choices=["narrow", "both", "broad"])
    parser_simple.add_argument("-o", "--output", dest = "out", help="output directory name", default="output")
    parser_simple.add_argument("-i", "--id", dest = "id", help="name for your chilin run, suggestion: use your factor name, e.g. ER or MYC", default="run")
...
def clean_up(workflow, conf):
    ...
    preserved.append(conf.prefix + "_conserv.pdf")
